...
# process management and is focused on user interaction/error checking. It has been updated for use in the C project 3 lab.
#
#Help Used:
...
function addProcesses {
    ...
    if [ $choice != n ]; then
        ...
        case $start in
            1) ping -i $speed "`hostname -i`" > pingData/"$file.txt" &;;
            2) ping -i $speed google.com > pingData/"$file.txt" &;;
        esac
...
function viewFile {
    ...
    #asks the user for the name of the file. lists the files in the folder
        echo " "
        echo "Enter the name of the ping file you wish to view (make sure to put the .txt)"
        ...
        #make sure proper name is entered
        while [[ ! "$file" =~ ^[[:alnum:].]+$ ]] || [ ${#file} -gt 14 ] ; do
            echo "invalid! please entere a proper name!"
            read -r -p ": " file
        done
...
#creates pingData folder if it doesn't exist
if [ ! -d "pingData" ]; then 
    mkdir pingData
fi
